...
const getDepartureData = async (accessToken) => {
  ...
  try {
    ...
    return {
      shortDirection: allInformation.results[0].serviceJourney.directionDetails.shortDirection,
      shortName: allInformation.results[0].serviceJourney.line.shortName,
      transportMode: allInformation.results[0].serviceJourney.line.transportMode,
...
export const fetchVasttrafikData = async () => {
  try {
    const accessToken = await getAccessToken();
    const departureData = await getDepartureData(accessToken);
